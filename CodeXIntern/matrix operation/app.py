...
def validate_matrix(matrix_data, rows, cols):
    try:
        matrix = []
        for i in range(rows):
            row = []
            for j in range(cols):
                key = f'matrix{i}{j}' if isinstance(matrix_data, dict) else f'matrix{i}{j}2'
...
def index():
    ...
    if request.method == 'POST':
        ...
        if operation:
            ...
            matrix2 = validate_matrix(request.form, rows, cols)
